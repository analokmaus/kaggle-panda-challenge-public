...
class BatchRenorm(torch.nn.Module):
    def __init__(
        ...
        # )
        self.running_mean = torch.zeros(num_features, dtype=torch.float)
        self.running_std = torch.ones(num_features, dtype=torch.float)
...
def convert_batchrenorm(module):
    if isinstance(module, torch.nn.DataParallel):
        mod = module.module
        mod = convert_batchrenorm(mod)

    mod = module
    for batchnorm, batchrenorm in zip([torch.nn.modules.batchnorm.BatchNorm1d,
    ...
    for name, child in module.named_children():
        mod.add_module(name, convert_batchrenorm(child))
